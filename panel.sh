...
#CPU
cpu_temp=$(echo -n $(sensors | grep "Core 0" | cut -b 16-19))
cpu_temp2=$(echo -n $(sensors | grep "Core 1" | cut -b 16-19))     
# small adjustments
right="$cpu_temp $separator^fg() $cpu_temp2 $separator^fg() $bat $separator^bg() $date $separator"
...
# wait 2 seconds for stalonetray
